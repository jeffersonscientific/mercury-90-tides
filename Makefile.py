...
class sourceFile(object):
  ...
  def __getModules(self):
    ...
    for lsave in lines:
      l=str.expandtabs(str.lower(lsave)[:-1],1)
      words=str.split(str.lstrip(l))
      if len(words) > 0:
        if words[0] == 'use':
          used.append(str.split(words[1],',')[0])
        if words[0] == 'module':
          if len (words) == 2 or words[1] != "procedure":
            defined.append(words[1])
        if words[0] == 'include':
          newstring = str.replace(words[1],'\'','')
          newstring = str.replace(newstring,'\"','')
          included.append(newstring)
      l=str.expandtabs(lsave[:-1],1)
      words=str.split(str.lstrip(l))
      if len(words) > 0:
        if words[0] == '#include':
          newstring = str.replace(words[1],'\"','')
...
# We delete all dependencies that are present several number of times.
  ...
  def __getFirstOrderDependence(self):
    ...
    for mod in self.used:
      ...
      obj = str.replace(source.filename,'.f90','.o')
